...
export class ChatGateway implements OnGatewayInit, OnGatewayConnection, OnGatewayDisconnect {
  ...
  async onChatJoin(
    ...
    this.server.to(`chat-${chat.chatId}`).emit('userJoin', `${client.user?.firstName.capitalize()} joined the chat`);

    console.log('onChatJoined');
  ...
  onChatLeave(
    ...
    this.server.to(`chat-${data.chatId}`).emit('userLeave', `${client.user?.firstName.capitalize()} left the chat`);
  }
  ...
  onTypingStart(
    ...
    this.server.to(`chat-${data.chatId}`).emit('onTypingStart');
  }
  ...
  onTypingStop(
    ...
    // console.log(data.chatId);
    // console.log(client.rooms);
    this.server.to(`chat-${data.chatId}`).emit('onTypingStop');
  }

  // @SubscribeMessage('onKeepAlive')
  // onKeepAlive(
  //   @MessageBody() data: any,
  //   @ConnectedSocket() client: AuthenticatedSocket,
  // ) {
  //   console.log('onKeepAlive');
  //   this.server.emit('onKeepAlive', 'websocket is alive');
  //   // client.emit('onKeepAlive', 'websocket is alive');
  // }

  // EVENTS
  @OnEvent('message.create')
  handleMessageCreateEvent(payload: { userId: string, message: any, chat: { chatId: string, creatorUserId: string, influencerUserId: string } }) {
    ...
    if (authorSocket) this.server.to(`chat-${chatId}`).emit('onMessageCreate', payload.message);
